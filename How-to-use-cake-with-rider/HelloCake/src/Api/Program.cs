...
builder.Services.AddValidatorsFromAssemblies(new[] { typeof(Program).Assembly , typeof(HelloCakeContext).Assembly});
builder.Services.AddMediatR(typeof(Program))
    .AddScoped(typeof(IPipelineBehavior<,>), typeof(LoggingBehaviour<,>))
    .AddScoped(typeof(IPipelineBehavior<,>), typeof(ValidationBehaviour<,>));

var connectionString = builder.Configuration.GetConnectionString(ConnectionsStringName);
builder.Services.AddDbContext<HelloCakeContext>(x => x.UseNpgsql(connectionString)).AddUnitOfWork<HelloCakeContext>();
...
{
    var context = serviceScope.ServiceProvider.GetService<HelloCakeContext>();
